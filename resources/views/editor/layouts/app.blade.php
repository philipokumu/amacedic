...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <title>{{ __('A-Quality Papers') }}</title>
    ...
    <body class="{{ $class ?? '' }}">
        @auth()
            ...
            @include('editor.layouts.page_templates.auth')
        @endauth
        @guest()
            @include('editor.layouts.page_templates.guest')
        @endguest
        ...
        <script>
          Dropzone.options.awesomeDropzone = {
                ...
                maxFiles: 6,
                parallelUploads: 1,
                acceptedFiles: ".jpg, .jpeg, .png, .doc, .pdf, .docx,.ppt,.xls,.xlsx, .csv, .pptx,.pptm,.txt,.zip,.mp3,.mp4,.wma,.mpg,.flv",
                addRemoveLinks: true,
