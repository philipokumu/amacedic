    ...
    <div>
      <ul class="nav nav-pills nav-pills-primary col-md-12" role="tablist" id="tabMenu">
          ...
          <a class="nav-link" href="#fileuploads" data-toggle="tab">
              <i class="material-icons">cloud</i> File uploads ({{$completedOrder->fileuploads->count()}})
              <div class="ripple-container"></div>
          </a>
          </li>
              <li class="nav-item">
    ...
    <div class="card-body -mt-8">
        ...
        <div class="tab-pane active" id="instructions">
          <table class="table">
            ...
            <tbody>
              ...
              <td>
                  {{substr($completedOrder['academicLevel'], 4)}}
              </td>
                <td>
                  <strong>Type of paper:</strong>
                </td>
                <td>
                  {{substr($completedOrder['typeOfPaper'],4)}}
                </td>
              </tr>
              <tr>
                <td>
              ...
              <tr>
                ...
                <td>
                  {{substr($completedOrder['subjectArea'],4)}}
                </td>
                <td>
                    <strong>Citation:</strong>
                </td>
                <td>
                  {{$completedOrder['citation']}}
                </td>
              </tr>
              <tr>
                <td>
                ...
                <td>
                  {{substr($completedOrder['spacing'],4)}}
                  @if (substr($completedOrder['spacing'],4)=='Double') ({{300 * $completedOrder['noOfPages']}} words) @elseif(substr($completedOrder['spacing'],4)=='Single') ({{600 * $completedOrder['noOfPages']}} words) @endif
                </td>
                <td>
              ...
              <tr>
                ...
                <td>
                  {{$completedOrder['powerpointSlides']}}
                </td>
                <td>
                  <strong>Sources:</strong>
                </td>
                <td>
                  {{$completedOrder['sources']}}
                </td>
              </tr>
              <tr>
                <td>
              ...
              <tr>
                ...
                <td>
                  @if (auth('admin')->check()||auth('web')->check())
                    <strong>{{substr($completedOrder['currency'],5)}} {{$completedOrder['totalPrice']}}</strong>
                  @endif
                @auth('writer') 
                @if ($completedOrder->originalWriterAmount == $completedOrder->writerAmount)
                  Ksh. {{$completedOrder['writerAmount']}}
                @else 
                  <span class="text-danger">Ksh. {{$completedOrder['writerAmount']}}</span>
                @endif
                @endauth
                @auth('editor') 
                  Ksh. {{$completedOrder['editorAmount']}}
                @endauth 
                </td>
                <td>
                ...
                <td>
                  @auth('admin')E{{$completedOrder['editor_id']}}@endauth
                </td>
                </tr>
                @auth('admin')
                  <tr>
                  ...
                  <tr>
                    ...
                    <td>
                      Ksh. {{$completedOrder['expensesAmount']}}
                    </td>
                    <td>
                      <strong>Balance:</strong>
                    </td>
                    <td>
                      Ksh. {{$completedOrder['balance']}}
                    </td>
                  </tr>
                @endauth
                <tr>
                  ...
                  {!!nl2br(e($completedOrder['paperInstructions']))!!}
                  </td>
                </tr>
              </tbody>
          </table>
        ...
        <div class="tab-pane" id="fileuploads">
          <table class="table">
            ...
            <tbody>
              @foreach ($completedOrder->fileuploads as $file)
                <tr>
                  <td>
                ...
                <td colspan="5">
                  <div class="form-group{{ $errors->has('fileUploads') ? ' has-danger' : '' }}">
                    <div class="form-group{{ $errors->has('fileUploads') ? ' is-invalid' : '' }} dropzone text-center py-5" id="awesomeDropzone">
                      ...
                      <input class="form-control" name="order_id" id="order_id" type="number" value={{$completedOrder->id}} readonly hidden/>
                  </div>
                </td>
              </tr>
            </tbody>
          ...
          <div class="tab-pane" id="messages">
            ...
            <span>
              @auth('web') <form method="POST" action="{{route('user.message.store',$completedOrder)}}" class="form-horizontal"> @endauth
              @auth('admin') <form method="POST" action="{{route('admin.message.store',$completedOrder)}}" class="form-horizontal"> @endauth
              @auth('writer') <form method="POST" action="{{route('writer.message.store',$completedOrder)}}" class="form-horizontal"> @endauth
              @auth('editor') <form method="POST" action="{{route('editor.message.store',$completedOrder)}}" class="form-horizontal"> @endauth
                @csrf
