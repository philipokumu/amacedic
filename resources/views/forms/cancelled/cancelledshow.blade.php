<div class="card">
  <div class="card-header modal-body -my-4 row">
    <span class="col-sm-8 text-danger">    
      <h4><strong><u>Client's reason for cancelling</u></strong></h4>
      <i>"{!!nl2br(e($cancelledOrder['clientCancelReason']))!!}"</i>
    </span>
  </div>
  @if($cancelledOrder['adminCancelReason'])
  <div class="table-responsive">
    <div class="card col-sm-12 text-success">    
      <h4><strong><u>Admin's response</u></strong></h4>
        <i>"{{$cancelledOrder['adminCancelReason']}}"</i>
    </div>
  </div>
  @endif
</div>
      ...
      <div>
        <ul class="nav nav-pills nav-pills-primary col-md-12" role="tablist" id="tabMenu">
            ...
            <a class="nav-link" href="#fileuploads" data-toggle="tab">
                <i class="material-icons">cloud</i> File uploads ({{$cancelledOrder->fileuploads->count()}})
                <div class="ripple-container"></div>
            </a>
            </li>
                <li class="nav-item">
      ...
      <div class="card-body -mt-8">
          ...
          <div class="tab-pane active" id="instructions">
            <table class="table">
              ...
              <tbody>
                ...
                <td>
                    {{substr($cancelledOrder['academicLevel'], 4)}}
                </td>
                  <td>
                    <strong>Type of paper:</strong>
                  </td>
                  <td>
                    {{substr($cancelledOrder['typeOfPaper'],4)}}
                  </td>
                </tr>
                <tr>
                  <td>
                ...
                <tr>
                  ...
                  <td>
                    {{substr($cancelledOrder['subjectArea'],4)}}
                  </td>
                  <td>
                      <strong>Citation:</strong>
                  </td>
                  <td>
                    {{$cancelledOrder['citation']}}
                  </td>
                </tr>
                <tr>
                  <td>
                  ...
                  <td>
                    {{substr($cancelledOrder['spacing'],4)}}
                    @if (substr($cancelledOrder['spacing'],4)=='Double') ({{300 * $cancelledOrder['noOfPages']}} words) @elseif(substr($cancelledOrder['spacing'],4)=='Single') ({{600 * $cancelledOrder['noOfPages']}} words) @endif
                  </td>
                  <td>
                ...
                <tr>
                  ...
                  <td>
                    {{$cancelledOrder['powerpointSlides']}}
                  </td>
                  <td>
                    <strong>Sources:</strong>
                  </td>
                  <td>
                    {{$cancelledOrder['sources']}}
                  </td>
                </tr>
                <tr>
                  <td>
                ...
                <tr>
                  ...
                  <td>
                    @if (auth('admin')->check()||auth('web')->check())
                      <strong>{{substr($cancelledOrder['currency'],5)}} {{$cancelledOrder['totalPrice']}}</strong>
                    @endif
                  @auth('writer') 
                    @if ($cancelledOrder->originalWriterAmount == $cancelledOrder->writerAmount)
                      Ksh. {{$cancelledOrder['writerAmount']}}
                    @else 
                      <span class="text-danger">Ksh. {{$cancelledOrder['writerAmount']}}</span>
                    @endif
                  @endauth
                  @auth('editor') 
                    Ksh. {{$cancelledOrder['editorAmount']}}
                  @endauth 
                  </td>
                  <td>
                  ...
                  <td>
                    @auth('admin')E{{$cancelledOrder['editor_id']}}@endauth
                  </td>
                  </tr>
                  @auth('admin')
                    <tr>
                    ...
                    <tr>
                      ...
                      <td>
                        Ksh. {{$cancelledOrder['expensesAmount']}}
                      </td>
                      <td>
                        <strong>Balance:</strong>
                      </td>
                      <td>
                        Ksh. {{$cancelledOrder['balance']}}
                      </td>
                    </tr>
                  @endauth
                    <td colspan="4">
                    <h4><strong>Paper instructions</strong></h4>
                    {!!nl2br(e($cancelledOrder['paperInstructions']))!!}
                    </td>
                  </tr>
                </tbody>
            </table>
          ...
          <div class="tab-pane" id="fileuploads">
            <table class="table">
              ...
              <tbody>
                @foreach ($cancelledOrder->fileuploads as $file)
                  <tr>
                    <td>
            ...
            <div class="tab-pane" id="messages">
              <div class="card modal-body">    
                ...
                <span>
                    @auth('web') <form method="POST" action="{{route('user.message.store',$cancelledOrder)}}" class="form-horizontal"> @endauth
                    @auth('editor') <form method="POST" action="{{route('editor.message.store',$cancelledOrder)}}" class="form-horizontal"> @endauth
                      @auth('writer') <form method="POST" action="{{route('writer.message.store',$cancelledOrder)}}" class="form-horizontal"> @endauth
                    @auth('admin') <form method="POST" action="{{route('admin.message.store',$cancelledOrder)}}" class="form-horizontal"> @endauth
                    @csrf
