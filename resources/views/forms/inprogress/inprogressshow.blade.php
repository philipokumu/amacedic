    ...
    <div>
      <ul class="nav nav-pills nav-pills-primary col-md-12" role="tablist" id="tabMenu">
          ...
          <a class="nav-link" href="#fileuploads" data-toggle="tab">
              <i class="material-icons">cloud</i> File uploads ({{$inprogressOrder->fileuploads->count()}})
              <div class="ripple-container"></div>
          </a>
          </li>
              <li class="nav-item">
        ...
        <div class="tab-content">
          <div class="tab-pane active" id="instructions">
            <table class="table">
              ...
              <tbody>
                ...
                <td>
                    {{substr($inprogressOrder['academicLevel'], 4)}}
                </td>
                  <td>
                    <strong>Type of paper:</strong>
                  </td>
                  <td>
                    {{substr($inprogressOrder['typeOfPaper'],4)}}
                  </td>
                </tr>
                <tr>
                  <td>
                ...
                <tr>
                  ...
                  <td>
                    {{substr($inprogressOrder['subjectArea'],4)}}
                  </td>
                  <td>
                      <strong>Citation:</strong>
                  </td>
                  <td>
                    {{$inprogressOrder['citation']}}
                  </td>
                </tr>
                <tr>
                  <td>
                  ...
                  <td>
                    {{substr($inprogressOrder['spacing'],4)}}
                    @if (substr($inprogressOrder['spacing'],4)=='Double') ({{300 * $inprogressOrder['noOfPages']}} words) @elseif(substr($inprogressOrder['spacing'],4)=='Single') ({{600 * $inprogressOrder['noOfPages']}} words) @endif
                  </td>
                  <td>
                ...
                <tr>
                  ...
                  <td>
                    {{$inprogressOrder['powerpointSlides']}}
                  </td>
                  <td>
                    <strong>Sources:</strong>
                  </td>
                  <td>
                    {{$inprogressOrder['sources']}}
                  </td>
                </tr>
                <tr>
                  <td>
                ...
                <tr>
                  ...
                  <td>
                    @if (auth('admin')->check()||auth('web')->check())
                      <strong>{{substr($inprogressOrder['currency'],5)}} {{$inprogressOrder['totalPrice']}}</strong>
                    @endif
                  @auth('writer') 
                    @if ($inprogressOrder->originalWriterAmount == $inprogressOrder->writerAmount)
                      Ksh. {{$inprogressOrder['writerAmount']}}
                    @else 
                      <span class="text-danger">Ksh. {{$inprogressOrder['writerAmount']}}</span>
                    @endif
                  @endauth
                  @auth('editor') 
                    Ksh. {{$inprogressOrder['editorAmount']}}
                  @endauth 
                  </td>
                  <td>
                  ...
                  @auth('admin')
                    ...
                    <tr>
                      ...
                      <td>
                        Ksh. {{$inprogressOrder['expensesAmount']}}
                      </td>
                      <td>
                        <strong>Balance:</strong>
                      </td>
                      <td>
                        Ksh. {{$inprogressOrder['balance']}}
                      </td>
                    </tr>
                  @endauth
                  <tr>
                    ...
                    {!!nl2br(e($inprogressOrder['paperInstructions']))!!}
                    </td>
                  </tr>
                </tbody>
            </table>
          ...
          <div class="tab-pane" id="fileuploads">
            <table class="table">
              ...
              <tbody>
                @foreach ($inprogressOrder->fileuploads as $file)
                  <tr>
                    <td>
                  ...
                  <td colspan="5">
                    <div class="form-group{{ $errors->has('fileUploads') ? ' has-danger' : '' }}">
                      <div class="form-group{{ $errors->has('fileUploads') ? ' is-invalid' : '' }} dropzone text-center py-5" id="awesomeDropzone">
                        ...
                        <input class="form-control" name="order_id" id="order_id" type="number" value={{$inprogressOrder->id}} readonly hidden/>
                    </div>
                  </td>
                </tr>
              </tbody>
            ...
            <div class="tab-pane" id="messages">
              ...
              <span>
                @auth('web') <form method="POST" action="{{route('user.message.store',$inprogressOrder)}}" class="form-horizontal"> @endauth
                @auth('admin') <form method="POST" action="{{route('admin.message.store',$inprogressOrder)}}" class="form-horizontal"> @endauth
                @auth('writer') <form method="POST" action="{{route('writer.message.store',$inprogressOrder)}}" class="form-horizontal"> @endauth
                @auth('editor') <form method="POST" action="{{route('editor.message.store',$inprogressOrder)}}" class="form-horizontal"> @endauth
                  @csrf
              ...
              <table class="table table-hover">
                ...
                <tbody>
                  @foreach ($historyOrders as $order)
                    @if ($inprogressOrder->id != $order->id)
                      <tr>
                        <td>
