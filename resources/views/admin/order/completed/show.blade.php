...
@extends('admin.layouts.app', ['activePage' => 'Completed', 'titlePage' => __('Completed order')])
      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header card-header-primary row">
            ...
            <span class="col-6">
              <a href="{{route('admin.inediting.index')}}" class='btn btn-info pull-right'>Back to list</a>
            </span>
          </div>
          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <form method="POST" action="{{route('admin.adjustwritamount.update',$completedOrder)}}" class="form-horizontal">
                  @csrf
                  @method('PATCH')
                  <div class="row">
                    ...
                    <div class="col-sm-7">
                      <div class="form-group{{ $errors->has('origWriterAmount') ? ' has-danger' : '' }}">
                        <input type="number" id="origWriterAmount" name="origWriterAmount" class="form-control{{ $errors->has('origWriterAmount') ? ' is-invalid' : '' }} mt-1" value="{{ $completedOrder->originalWriterAmount }}" readonly aria-readonly="true"/>
                  ...
                  <div class="row">
                    ...
                    <div class="col-sm-7">
                      <div class="form-group{{ $errors->has('writerAmount') ? ' has-danger' : '' }}">
                        <input type="number" id="writerAmount" name="writerAmount" class="form-control{{ $errors->has('writerAmount') ? ' is-invalid' : '' }} mt-1" value="{{$completedOrder->writerAmount }}" min="0" max="{{ $completedOrder->originalWriterAmount }}"/>
                        @if ($errors->has('writerAmount'))
                        <span id="writerAmount-error" class="error text-danger" for="input-writerAmount">{{ $errors->first('writerAmount') }}</span>
                        @endif
                      </div>
