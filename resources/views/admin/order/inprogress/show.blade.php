...
@extends('admin.layouts.app', ['activePage' => 'In-progress', 'titlePage' => __('Inprogress order')])
      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header card-header-primary row">
            ...
            <span class="col-6">
              <a href="{{route('admin.inprogress.index')}}" class='btn btn-info pull-right'>Back to list</a>
            </span>
          </div>
              ...
              <tr>
                <th>
                  ...
                  <span class="pull-right">
                    ...
                    <a href="{{route('admin.bids.index',$inprogressOrder)}}" class='btn btn-danger'>Reassign</a>
                    <p>
            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form method="POST" action="{{route('admin.adjustdeadline.update',$inprogressOrder)}}" class="form-horizontal">
                    @csrf
                    @method('PATCH')
                    <div class="row">
          ...
          <div class="modal fade" id="changeWAmountModal" tabindex="-1" role="dialog" aria-labelledby="changeWAmountModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form method="POST" action="{{route('admin.adjustwritamount.update',$inprogressOrder)}}" class="form-horizontal">
                    @csrf
                    @method('PATCH')
                    <div class="row">
                      ...
                      <div class="col-sm-7">
                        <div class="form-group{{ $errors->has('origWriterAmount') ? ' has-danger' : '' }}">
                          <input type="number" id="origWriterAmount" name="origWriterAmount" class="form-control{{ $errors->has('origWriterAmount') ? ' is-invalid' : '' }} mt-1" value="{{ $inprogressOrder->originalWriterAmount }}" readonly aria-readonly="true"/>
                    ...
                    <div class="row">
                      ...
                      <div class="col-sm-7">
                        <div class="form-group{{ $errors->has('writerAmount') ? ' has-danger' : '' }}">
                          <input type="number" id="writerAmount" name="writerAmount" class="form-control{{ $errors->has('writerAmount') ? ' is-invalid' : '' }} mt-1" value="{{$inprogressOrder->writerAmount }}" min="0" max="{{ $inprogressOrder->originalWriterAmount }}"/>
                          @if ($errors->has('writerAmount'))
                          <span id="writerAmount-error" class="error text-danger" for="input-writerAmount">{{ $errors->first('writerAmount') }}</span>
                          @endif
                        </div>
