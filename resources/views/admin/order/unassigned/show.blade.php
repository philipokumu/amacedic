...
@extends('admin.layouts.app', ['activePage' => 'Available', 'titlePage' => __('Available order')])
      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header card-header-primary row">
            ...
            <span class="col-6">
              <a href="{{route('admin.unassigned.index')}}" class='btn btn-info pull-right'>Back to list</a>
            </span>
          </div>
              ...
              <tr>
                <th>
                  <span class="pull-right">
                    <button type="submit" class="btn btn btn-warning" data-toggle="modal" data-target="#changeWAmountModal" data-original-title="submit" title="submit">
                  ...
                  <span class="pull-right">
                    <a href="{{route('admin.bids.index',$order)}}" class='btn btn-success'>View bids ({{$order->bids->count()}})</a>
                  </span>
                </th>
              </tr>
            </thead>
          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <form method="POST" action="{{route('admin.adjustwritamount.update',$order)}}" class="form-horizontal">
                  @csrf
                  @method('PATCH')
                  <div class="row">
                    ...
                    <div class="col-sm-7">
                      <div class="form-group{{ $errors->has('origWriterAmount') ? ' has-danger' : '' }}">
                        <input type="number" id="origWriterAmount" name="origWriterAmount" class="form-control{{ $errors->has('origWriterAmount') ? ' is-invalid' : '' }} mt-1" value="{{ $order->originalWriterAmount }}" readonly aria-readonly="true"/>
                  ...
                  <div class="row">
                    ...
                    <div class="col-sm-7">
                      <div class="form-group{{ $errors->has('writerAmount') ? ' has-danger' : '' }}">
                        <input type="number" id="writerAmount" name="writerAmount" class="form-control{{ $errors->has('writerAmount') ? ' is-invalid' : '' }} mt-1" value="{{$order->writerAmount }}" min="0" max="{{ $order->originalWriterAmount }}"/>
                        @if ($errors->has('writerAmount'))
                        <span id="writerAmount-error" class="error text-danger" for="input-writerAmount">{{ $errors->first('writerAmount') }}</span>
                        @endif
                      </div>
